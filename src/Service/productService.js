...
const updateProduct = async ({
  ...
  try {
    ...
    const addFileDeletion = async (oldFile, newFilePath = null) => {
      if (!oldFile || !oldFile.url) return;

      const oldPath = oldFile.url.replace(/\\/g, '/');
      const oldName = oldPath.split('/').pop();

      let newName = null;

      if (newFilePath && typeof newFilePath === 'string') {
        const newPath = newFilePath.replace(/\\/g, '/');
        newName = newPath.split('/').pop();
      }

      // If same filename, don't delete
      if (newName && oldName === newName) {
        return;
      }

      const publicId = getFileId(oldFile.url);

      if (publicId) {
        filesDeletions.push(destroyFile(publicId));
      }
    };
    ...
    if (imageFiles && imageFiles.length > 0) {
      ...
      productData.imagesToDelete.length > 0
    ) {
      // Find the images to be deleted
      const imagesToDelete = await ImageFiles.find({
        _id: { $in: productData.imagesToDelete },
    ...
    if (pdfFile) {
      ...
      if (existingProduct.pdf) {
        ...
        if (existingPdf) {
          await addFileDeletion(existingPdf, pdfFile.path);
        }
      ...
      const existingPdf = await PdfFiles.findById(existingProduct.pdf);
      if (existingPdf) {
        await addFileDeletion(existingPdf);
      }
      tasks.push(PdfFiles.findByIdAndDelete(existingProduct.pdf));
      newPdfId = null;
    }
    ...
    if (videoFile) {
      ...
      if (existingProduct.video) {
        ...
        if (existingVideo) {
          await addFileDeletion(existingVideo, videoFile.path);
        }
      ...
      const existingVideo = await VideoFiles.findById(existingProduct.video);
      if (existingVideo) {
        await addFileDeletion(existingVideo);
      }
      tasks.push(VideoFiles.findByIdAndDelete(existingProduct.video));
      newVideoId = null;
    }
    ...
    await Promise.all([...tasks, ...filesDeletions]);
